...
describe('Message', () => {
  ...
  it('renders component with proper class depending on the type prop', () => {
    ...
    expect(screen.getByRole('listitem').classList).toContain('messages-list__element--incoming');

    rerender(<Message text={mockText} type="outgoing" />);
    expect(screen.getByRole('listitem').classList).toContain('messages-list__element--outgoing');
  });
});
